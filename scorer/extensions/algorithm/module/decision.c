...
Decision *create_decision(char const *id, char const *notification_id,
                          int selected, long timestamp_sent,
                          long timestamp_selected) {
  ...
  decision->timestamp_sent = timestamp_sent;
  decision->timestamp_selected = timestamp_selected;
...
Decision *parse_python_decision_obj(PyObject *obj) {
  ...
  PyObject *timestamp_sent_obj = PyDict_GetItemString(obj, "timestamp");
  PyObject *timestamp_response_obj =
      PyDict_GetItemString(obj, "response_timestamp");

  if (!id_obj || !notification_obj || !selected_obj || !timestamp_sent_obj ||
      !timestamp_response_obj) {
  ...
  const double timestamp = PyFloat_AsDouble(timestamp_sent_obj);
  const double response = PyFloat_AsDouble(timestamp_response_obj);
  ...
  Decision *decision =
      create_decision(id, notification_id, selected, timestamp, response);
  if (decision == NULL) {
...
NotificationArray *parse_python_notification_dict(PyObject *dict) {
  ...
  for (Py_ssize_t i = 0; i < keys_length; i++) {
    ...
    Notification *notification =
        create_notification(notification_id, 0, value, -1);
...
void print_decision_list(DecisionArray *list) {
  printf("\n--- Decision List ---\n");

  for (size_t i = 0; i < list->length; i++) {
    Decision *decision = list->array[i];

    printf("Decision: id[%s] | Notification: [%s] | Timestamp: [%ld] | "
           "Response Timestamp: [%ld]\n",
           decision->id, decision->notification_id, decision->timestamp_sent,
           decision->timestamp_selected);

    printf("\n--- Probabilities ---\n");
    print_notification_list(decision->probabilities);
    printf("\n--- Probabilities ---\n");
  }
