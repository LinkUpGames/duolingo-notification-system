...
func createDecisionEvent(decisionID string, selected bool, timestamp int64, db *db.DB) bool {
	...
	// Updated
	query = fmt.Sprintf("update scores s set timestamp = %d from decisions d where d.id = '%s' and s.id = d.notification_id and s.user_id = d.user_id;", timestamp, decisionID)
	status = db.SetEntry(query)
