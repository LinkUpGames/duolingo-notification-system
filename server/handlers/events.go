...
// GetDecisionEventHandler Get the Event log that is associated with this decision log
func GetDecisionEventHandler(ctx *cmd.AppContext, w http.ResponseWriter, r *http.Request) {
	// Context
	db := ctx.Ctx.Value(cmd.DATABASE).(*db.DB)
	...
	// Fetch
	log := events.GetDecisionEvent(id, db)
